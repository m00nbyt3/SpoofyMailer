...
void s_html(char to[], char from[], char name[], char subject[])
{
    FILE *fichero;
    ...
    while(0 == 0) {
	    printf("Introduzca el nombre del fichero HTML");
	    printf("(ejemplo: mensaje.html): ");
	    scanf("%s", file);
	    
	    fichero = fopen(file,"r");
	    if (fichero == NULL) {
            printf("\nEl fichero no existe!!!\n");
            printf("Recuerda meter el fichero en la misma carpeta que el programa\n\n");
            } else {
            break;
        }         
    }
    ...
    strcat(send, "' -o message-content-type=html -o message-file=");
    strcat(send, file);
